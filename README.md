...
| **Firmware crossed its own seam, three times** | `fw_bringup.c` called the hardware-side backend to raise `STAT_AGC_CONV`, and `fw_adapt.c` read-modify-wrote `REG_ADAPT_STAT`, which the map declares RO -- using a bit constant from a *different* register's namespace, so it meant the right thing only by numerical accident. Both wrote values that **nothing read**, which is why nothing ever failed. The seam is the central claim of the project and it was only ever violated to produce dead data. CI now greps `src/fw_*.c` for it, because a claim that lives in a header is a claim that drifts. |
| The third crossing reset the whole register file, and hid a multi-lane bug while it did | `fw_init()` called the hardware-side whole-file reset -- the same entry point the header reserves for the model and the tests. On silicon that is the reset controller's job and it happens before firmware runs; there is no such call for firmware to make. It was also latent breakage: the reset clears EVERY lane's aperture, so a supervisor bringing up eight lanes in turn wiped lanes 0 through 6 while initialising lane 7. Harmless today only because that function writes no configuration -- a property of the current code, not a guarantee -- which is exactly the kind of accident that stops being harmless when someone adds a line. Moved to platform init in the applications; the CI grep now covers it too. |
